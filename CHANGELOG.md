...
### Fixed
- `from deepextractor.training.train_fn import train_fn` no longer raises `ModuleNotFoundError` — dataset classes are now imported lazily inside `get_loaders()` rather than at module level

## [0.1.3] - 2026-04-01

### Fixed
- Glitch reconstruction tutorial notebook now resolves asset paths via `importlib.resources` — works correctly when run outside the repo after `pip install`

## [0.1.2] - 2026-04-01

### Fixed
- Re-saved bundled scalers with scikit-learn 1.8.0 to suppress `InconsistentVersionWarning` on load
